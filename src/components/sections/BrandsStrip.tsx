...
function MarqueeRow({
  ...
  const doubled = Array(6).fill(items).flat(); // 6x duplication for ultrawide coverage
  return (
    <div className="relative flex w-full overflow-hidden">
      <motion.div
        className="flex w-max shrink-0 items-center"
        animate={{
          x: direction === "left" ? ["0%", "-16.666%"] : ["-16.666%", "0%"],
        }}
        transition={{
          x: {
            repeat: Infinity,
      ...
      >
        {doubled.map((brand, i) => (
          ...
          >
            <div className="flex items-center justify-center px-6 opacity-40 transition-opacity duration-300 hover:opacity-80 sm:px-8 md:px-12 lg:px-16 xl:px-20">
